...
$active = homesweet_get_config('property_header_active_tab', 'gallery');
?>
...
<div class="tabs-gallery-map">
		...
		<?php if ( ! empty( $gallery ) ) : ?>
			<li class="<?php echo esc_attr($active == 'gallery' ? 'active' : ''); ?>">
				<a href="#tab-gallery-map-gallery" data-toggle="tab">
	                ...
	                <span><?php esc_html_e('View Photos', 'homesweet'); ?></span>
	            </a>
	        </li>
        <?php endif; ?>
	...
	<div class="tab-content tab-content-descrip">
		...
		<?php if ( ! empty( $map_location ) && 2 == count( $map_location ) ) : ?>
			<div id="tab-gallery-map-map" class="wrapper-box-shadow tab-pane <?php echo esc_attr($active == 'map' ? 'active' : ''); ?>">
				<?php echo Realia_Template_Loader::load('single/map'); ?>
			</div>

			<div id="tab-gallery-map-mapview" class="wrapper-box-shadow tab-pane <?php echo esc_attr($active == 'mapview' ? 'active' : ''); ?>">
				<div id="single-tab-property-street-view-map" style="height: 400px"></div>
			</div>
		<?php endif; ?>

		<?php if ( ! empty( $virtual_tour ) ) : ?>
			<div id="tab-gallery-map-virtual_tour" class="wrapper-box-shadow tab-pane <?php echo esc_attr($active == 'virtual_tour' ? 'active' : ''); ?>">
