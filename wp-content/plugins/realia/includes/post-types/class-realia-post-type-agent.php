...
class Realia_Post_Type_Agent {
	...
	public static function fields( array $metaboxes ) {
		...
		$metaboxes[ REALIA_AGENT_PREFIX . 'contact_details' ] = array(
			'id'              	=> REALIA_AGENT_PREFIX . 'contact_details',
		...
		foreach( $social_networks as $key => $title ) {
			$fields[] = array(
				'name'              => $title,
				'id'                => REALIA_AGENT_PREFIX . 'social_' . $key,
				'type'              => 'text_url',
			);
		}

		$metaboxes[ REALIA_AGENT_PREFIX . 'social_networks' ] = array(
			'id'              	=> REALIA_AGENT_PREFIX . 'social_networks',
		...
		$metaboxes[ REALIA_AGENT_PREFIX . 'agencies' ] = array(
			'id'              	=> REALIA_AGENT_PREFIX . 'agencies',
			'title'           	=> __( 'Agencies', 'realia' ),
			'object_types'    	=> array( 'agent' ),
			'context'         	=> 'normal',
			'priority'        	=> 'high',
			'show_names'      	=> true,
	...
	public static function fields_front( array $metaboxes) {
		if ( ! is_admin() ) {
			...
			if ( count( $social_networks ) > 0 ) {
				$fields[] = array(
					'name'              => __( 'Social networks', 'realia' ),
					'id'                => REALIA_AGENT_PREFIX . 'social_networks_title',
					'type'              => 'title',
				);
	...
	public static function process_agent_form() {
		if ( ! isset( $_POST['submit-cmb'] ) && ! empty( $_POST['post_type'] ) && 'agent' == $_POST['post_type'] || empty( $_POST[ REALIA_AGENT_PREFIX . 'title' ] ) ) {
			return;
		}

		// Setup and sanitize data
		$title = sanitize_text_field( $_POST[ REALIA_AGENT_PREFIX . 'title' ] );

		if ( empty( $title ) ) {
			return;
		}

		$description = wp_kses( $_POST[ REALIA_AGENT_PREFIX . 'description' ], '<b><strong><i><em><h1><h2><h3><h4><h5><h6><pre><code><span>' );
